...
export default class QuillPricebookCustom extends LightningElement {
  ...
  set customBook(value) {
    ...
    this.pricebook = value ? JSON.parse(JSON.stringify(value)) : null;
  }
  ...
  set standardBook(value) {
    ...
    this.standardPricebook = value ? JSON.parse(JSON.stringify(value)) : null;
  }
  ...
  setDiscountTypeForAll(e) {
    ...
    const inputs = this.template.querySelectorAll(
      'table[data-selected] tbody input[type="checkbox"]'
    );
  ...
  setDiscountValueForAll(e) {
    if (e.key === "Enter") {
      ...
      const event = new CustomEvent("change");
      const inputs = this.template.querySelectorAll(
        'table[data-selected] tbody input[type="number"]'
      );
      inputs.forEach((input) => {
        input.value = value;
        input.dispatchEvent(event);
  ...
  changeDiscountType(e) {
    ...
    const input = e.currentTarget.parentNode.parentNode.querySelector(
      'input[type="number"]'
    );

    const entry = this.pricebook.PricebookEntries.records.find(
      ({ Id }) => Id === entryId
    );
  ...
  recalculatePrice(entry, standardPrice) {
    if (entry.Is_Percent_Discount__c) {
      entry.UnitPrice = (1 - entry.Discount__c / 100) * standardPrice;
    } else {
      entry.UnitPrice = standardPrice - entry.Discount__c;
    }
  ...
  updatePricebook() {
    this.dispatchEvent(
      new CustomEvent("updatepricebook", { detail: this.pricebook })
    );
  }
}
